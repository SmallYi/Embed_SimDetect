...
# OLDTB : test0520
DB_INFO = {'host':'127.0.0.1','port':3306,'DB':'YJ_TEST','TB':'test0523'}
folder = 'F:/Study/510/DocYJ/DataBase/Test20190520/'
...
class Date_Analysis():
    ...
    # 数据分析主过程
    def MainAnalysis(self,binary_addr,mat_addr,folder,funcname_f):
        i = 0
        j = 0
        ...
        try:
            ...
            for each_binary in binary_contents:
                if i < y_max:
                    ...
                    func_addr = folder + funcname_f + binary_name + '.txt'
                    ...
                    for each_func in func_contents:
                        if j < z_max:
                            ...
                            #     pass
                            # else:
                            temp = self.DataAccuray(s_data[:,i,j][0:60])
                            str_data = temp.astype(str)
                            feature = "-".join(str_data)
                            print(binary_name,func_name,feature,len(feature))
...
class LSHAnalysis():
    ...
    def searchnew(self,test,model,s_key):
        ...
        return totaly
    # key表示获得相似feature的个数
    def Mainfunc(self,mat_addr,result_folder,binary_file,key):
        ...
        compareDict = {'core_dir_arm_o0':16,'core_dir_arm_o1':17,'core_dir_arm_o2':18,'core_dir_arm_o3':19,
                       ...
                       'busybox_arm_o0':0,'busybox_arm_o1':1,'busybox_arm_o2':2,'busybox_arm_o3':3,
                       'openssl_arm_o0':84, 'openssl_arm_o1':85,'openssl_arm_o2':86,'openssl_arm_o3':87,
                       'sqlite_arm_o0':0,'sqlite_arm_o1':1, 'sqlite_arm_o2':2,'sqlite_arm_o3':3,
                       'sqlite_x86_o0': 8, 'sqlite_x86_o1': 9, 'sqlite_x86_o2': 10, 'sqlite_x86_o3': 11,
                       'sqlite_mips_o0': 4, 'sqlite_mips_o1': 5, 'sqlite_mips_o2': 6, 'sqlite_mips_o3': 7,
                       'core_dir_mips_o0': 4, 'core_dir_mips_o1': 5, 'core_dir_mips_o2': 6, 'core_dir_mips_o3': 7,
                       'core_dir_x86_o0': 12, 'core_dir_x86_o1': 13, 'core_dir_x86_o2': 14, 'core_dir_x86_o3': 15,
                       'openssl_mips_o0': 88, 'openssl_mips_o1': 89, 'openssl_mips_o2': 90, 'openssl_mips_o3': 91,
                       'openssl_x86_o0': 92, 'openssl_x86_o1': 93, 'openssl_x86_o2': 94, 'openssl_x86_o3': 95,
                       }
        ...
        #core 只针对DIR
        imodel_name = 'sqlite_x86_o2'
        imodel_BIN_name = 'sqlite_x86_o2'
        ...
        itest_name = 'sqlite_x86_o1'
        itest_BIN_name = 'sqlite_x86_o1'
        ...
        model_num = 0
        test_num = 0
        for j in range(imodel_s_n[1]):
            data[:, model_num] = svec[:, imodel, j]
            model_num = model_num + 1
        for j in range(itest_s_n[1]):
    ...
    #    output_total = open(result_folder + 'result_total.txt', 'w')
        ...
        # Inmodel_NUM = 0.0
        output = open(result_folder + 'Key1Between' + '.txt', 'a')
...
if __name__ == "__main__":
    ...
    # key = 10
    # DOlsh = LSHAnalysis()
    # DOlsh.Mainfunc(folder+mat_file,folder+select_result_folder,folder+binary_file,key)
...
# 9172,2284
    ...
    # 查询某条记录
    # sql = "select * from test0520 where function_name = 'fts3EvalStartReaders'"
    # 查询记录数
    ...
    # 数据分析并保存数据库
    # DOAnalysis.MainAnalysis(folder+binary_file,folder+mat_file,folder,funcname_f)

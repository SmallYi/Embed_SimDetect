...
class LSHAnalysis():
    ...
    def Mainfunc(self,mat_addr,result_folder,binary_file,key):
        ...
        #core 只针对DIR
        imodel_name = 'sqlite_mips_o0'
        imodel_BIN_name = 'sqlite_mips_o0'
        ...
        for j in range(imodel_s_n[1]):
            data[:, model_num] = svec[:, imodel, j][0:60]
            model_num = model_num + 1
        for j in range(itest_s_n[1]):
            test[:, test_num] = svec[:, itest, j][0:60]
            test_num = test_num + 1
    ...
    #    output_total = open(result_folder + 'result_total.txt', 'w')
        ...
        # Inmodel_NUM = 0.0
        output = open(result_folder + 'KeyBetweenBySumFindStr0523' + '.txt', 'a')
